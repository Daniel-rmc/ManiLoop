...
ManiLoop 评测的是完整策略配置：模型、传感输入表示、动作能力与本地控制器。首版以可运行的跨机器人闭环为基础，通过组合独立组件扩展，避免先构建庞大的继承体系。
...
- `Agent` 协议接收文字、传感观测、图像、历史和深度查询结果。`LLMAgent` 组合 API provider；未来可替换 provider 或接本地 / 远端 VLA 推理服务。
...
首版仍保留旧的 `arx5_demo` 导入与启动兼容层。核心 Python 包安装到 `src/maniloop`；资源跟随包一起安装，运行结果写到用户指定目录。
...
仿真真值只用于初始化、独立评分与离线物理测试。记录文件可以包含供研究者审计的场景配置与评分，**这些记录不回流至模型输入**。相机外参和 TCP 来自标定 / 机器人运动学，物体结构化表示未来必须从传感数据计算，并记录算法版本、误差与来源。

这是代码接口上的隔离，不是针对不可信 Python 插件的进程安全沙箱。未来加载第三方策略代码时，应使用独立进程或远端推理服务，仅交换序列化传感包。

## 动作与时序

`Action` 统一表达 TCP 增量、带关节名称的绝对关节目标、夹爪或等待；采用米、弧度、秒，坐标系明确为机器人基座或后端声明的世界坐标系。LLM 的单步 JSON 在边界转为 `Action`；深度查询和结束声明是控制消息。

`ActionChunk` 包含观测 ID、一组 `Action` 和采样间隔。首版限制为 1–64 个采样，间隔 0.02–1 秒。模拟 VLA 输出两个小幅关节目标；`ChunkExecutor` 按仿真时钟逐个校验和执行，然后等待停稳并重新观察。没有静默单位转换、动作裁剪或任意代码执行。
...
批量实验限制决策次数、仿真时间和墙钟时间。自建桌面任务在模型声明结束后允许最多 1.2 秒固定静置窗口完成稳定性判定，并受剩余仿真预算限制。模型声明结束不等于独立评分成功。
...
模拟策略的记录标记 `is_mock=true`。物理测试中的已知坐标只用于证明场景可操作，不能作为模型自主成功率。不发布跨模型排行榜；真实策略的小规模验证见工作记录，真机尚未验证。

## 下一次扩展怎么做

1. **新机械臂**：添加模型与来源许可证；新增 `RobotSpec` 的关节、驱动和传感映射；在场景组装中加入必要 TCP / 相机适配；跑重置、夹爪、动作拒绝与接触任务测试。当前 CLI 的候选列表也需更新，尚未引入自动插件发现。
2. **新场景 / 物体**：把网格、纹理和 MJCF 放入相应资源目录，编写布局组装；避免将坐标硬编码到策略中。两个现有布局使用同一桌面和方块，仅位置不同。
3. **新任务**：提供指令、初始化参数、每步状态更新和独立评分；首先用离线物理测试证明至少一条合法操作路径，再运行模型实验。
4. **新 LLM 供应商**：实现 provider，保持 Agent 输入与 `Action` 语义不变。当前仅实现 Responses 兼容服务；列出某模型不代表服务支持图像和结构化输出。
5. **新 VLA / 视觉策略**：实现 `Agent.reset/decide`；将模型原始归一化输出显式转换到 SI 单位和命名关节 / TCP 动作；记录图像预处理、动作缩放、时域和频率。先用已录制输出验证适配，再连接独立推理进程。
6. **结构化表示**：增加表示实现，从 RGB-D、标定与历史计算；输出必须有传感来源，禁止读取物体真值补齐结构化信息。
7. **学习模式**：另建学习运行器、数据切分和训练状态记录。先固定评测集与防止训练泄漏的边界，再加入跨 episode 更新。

当新增第三种机器人、第二种表示或第二种 provider 时，再从重复代码提炼注册机制和配置工厂。以扩展实例验证抽象，维持浅层组合和清晰的对象所有权。
...
资源保留在可选上游安装中，不把大体积官方 assets 混进自建 `assets/`。源码版本、初始化文件摘要、控制参数、运行依赖与图像处理进入 manifest；不同协议明确分组。安装与比较限制见 [LIBERO.md](LIBERO.md)，演进记录见 [worknotes/worknote.md](worknotes/worknote.md)。
...
首次真实云端测试的超时、场景生命周期和控制步诊断，以及目标执行器／观测表示的建议见 [诊断与 v2 设计](design/LLM_LOOP_V2.md)。P1/P2 已实现：三种只读连接诊断、实际请求参数与失败耗时记录、`llm_rgb512` 观测和 `controllers/target.py` 的固定目标伺服。`EpisodeRunner` 记录执行终态反馈；CLI 与网页共用运行器。原生动作配置仍保留，按动作接口和观测协议分组。真实云端任务评测另行开展。
