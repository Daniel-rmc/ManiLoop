...
- 真实 GPT-6 完整任务已通过官方评分：46 次决策、706 控制步，完整视频独立解码为 707 帧、20 fps、35.35 秒。过程和成功触发后的终止边界见[成功 episode 报告](research/2026-09-16-gpt6-success-episode.md)。
- 9 项离线检查通过：连续索引、篡改检测、评分步数一致性、失败导出限制、输出目录防覆盖和不支持组合拒绝。
- 真实 LIBERO 录制开／关各执行相同 8 步，本体轨迹逐位一致；录制 9 组双相机，终态评分与环境一致。
- CLI 离线 fixture 尝试成功保存完整记录，并导出 MP4；独立解码确认 3 帧、20 fps、0.15 秒。这是录像链路验收，不代表 GPT 任务成功。
