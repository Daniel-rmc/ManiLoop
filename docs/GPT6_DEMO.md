...
模型的观测与执行反馈仅基于真实机器人上也可获得的传感信息：相机图像和机器人本体状态。不会向模型提供仿真器额外的真值信息，例如物体位姿、接触真值、奖励或任务成功信号。控制器依据允许的本体观测与模型上一条指令计算执行反馈，通过下一次观测的 `last_feedback` 和近期动作历史提供给模型；`paired` 模式还在前后状态转换中携带这份反馈。

其中 `position_error_m = ‖执行后的实测 TCP 位置 − 上一条指令确定的 TCP 目标位置‖`，由控制器计算后提供给模型，网页换算为毫米显示为“TCP误差”。模型自身的指令与机器人执行后的本体观测足以计算这项残差，**无需物体位置真值；它也不是夹爪与目标物体的距离**。目标位置由动作开始时的末端位置加上模型请求的位移得到；夹爪动作则保持当前末端目标。
